...
function valuesFor(age: GameProps["age"], round: number) {
  const limit = age === "toddler" ? 3 : age === "preschool" ? 5 : 8;
  const left = (round * 2) % limit + 1;
  const right = (round * 3 + 1) % limit + 1;
  return { left, right };
}

function choicesFor(answer: number) {
  const candidates = [answer, Math.max(1, answer - 1), answer + 1, Math.max(1, answer - 2)];
  return Array.from(new Set(candidates)).concat([answer + 2, answer + 3]).slice(0, 4).sort((a, b) => ((a * 7) % 9) - ((b * 7) % 9));
}
...
export function AdditionGame({ age, language, onComplete }: GameProps) {
  ...
  const choices = choicesFor(answer);
  ...
  return (
    <div className="addition-game">
      ...
      <div className="addition-progress">{Array.from({ length: 5 }, (_, index) => <span key={index} className={index < round ? "done" : index === round ? "active" : ""} />)}</div>
